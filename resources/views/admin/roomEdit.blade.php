...
<main class="form-signin w-100 m-auto">
    <form method="POST" action="{{ route('room.update', $room->id) }}">
        ...
        <div class="form-floating mt-3">
            <input type="text" name="status" class="form-control" id="floatingInput" value="{{ $room->statusRoom }}">
        ...
        <div class="form-floating mt-3">
            <select name="idTypeRoom" class="form-select">
                @foreach ($types as $type)
                    <option 
                    @if ($type->id == $room->typeRoom_idTypeRoom)
                        selected
                    @endif value="{{$type->id}}">{{$type->typeRoom}}</option>
                @endforeach
              </select>
        </div>
